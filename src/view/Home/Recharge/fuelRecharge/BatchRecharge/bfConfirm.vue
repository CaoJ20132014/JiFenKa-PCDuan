<template>
    <div class="b-confirm">
            ...
            <div class="table-con">
                <ul>
                    <li v-for="(item,index) of tableList" :key="item.id" :class="[((index + 1) % 2) == 0 ? 'bg' : '']">
                        ...
                        <div v-text="(item.cash-0).toFixed(3)"></div>
                        <div v-text="(item.koukuan-0).toFixed(3)"></div>
        ...
        <div class="buttons">
            ...
            <p>订单总览：共<span v-text="tableList.length"></span>条订单，总金额：<span v-text="totalAll"></span>元，实扣金额：<span v-text="total"></span>元</p>
        </div>
...
<script>
    import recordtable from '../components/table.vue';
    export default {
        components:{
            recordtable
        },
        ...
        computed:{
            total(){
                ...
                return (totalMoney-0).toFixed(3);
            },
            totalAll(){
                let totalAllMoney = 0;
                this.tableList.forEach(item => {
                    totalAllMoney += (item.cash - 0);
                });
                return (totalAllMoney-0).toFixed(3);
            }
        },
        methods: {
            mPay(){
...
<style lang="less" scoped>
    @import '../../../../../assets/less/recharge/fuelRecharge/batchRecharge/bfConfirm.less';
</style>